...
| `RoboBrain2`       | [RoboBrain 2.0 / 2.5](https://github.com/FlagOpen/RoboBrain2.5) による長期計画とマルチモーダル推論モデル         | [`BAAI/RoboBrain2.5-4B`](https://huggingface.co/BAAI/RoboBrain2.5-4B)                                                                                                                                     | `name`、`checkpoint`、`source`、`init_timeout`                                                                                                                       |
...
门控和受限模型会在初始化时自动检测（两个平台均支持）。仅当检查点确实需要凭据时才会提示：初始化会失败并给出所需令牌的设置说明 —— HuggingFace Hub 使用 `HF_TOKEN`（或 `huggingface-cli login`），ModelScope 使用 `MODELSCOPE_API_TOKEN`（或 `modelscope login`）。
